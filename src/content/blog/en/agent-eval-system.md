...
Model evaluation usually starts from a question and a reference answer. Agents are different. An agent works in an environment: it has to understand an incomplete request, plan a path, call tools, make judgments inside changing data sources, and deliver something a person can pick up. For a product in production, the ultimate question is whether it genuinely helped the user get the work done.

So the goal of agent evaluation shouldn't just be ranking models. It's building a product language: when we say "this version is better," everyone knows which tasks it got better on, on what evidence, and whether that holds up in real business use.
...
Plenty of evaluation projects open by debating metrics or judges. The first thing to do is actually to break the product's tasks apart. Different tasks have different ground truth: some have a definite answer, some can only be judged by a domain expert, and some can't be judged until user behavior has happened.

Start from the concrete business. First the recruiting workflow: from requirement clarification, through talent research and search, initial screening, outreach and follow-through, to interviews, selection and hire. That is the business map the product serves. Against it sit the evaluation tasks: along the dimensions of ground truth and delivery shape, we can carve out the units of work an agent can be evaluated on independently.
...
Concretely, the evaluation task types for a recruiting agent. They all look like "finding people" on the surface, but the evaluation configs are completely different.
...
Task type is only the first layer of the dataset. The second layer is the finer business scenario: industry, function, seniority, region, and candidate-pool size. An executive role may have a pool under a hundred people — closer to an enumerable mapping problem; a high-volume mid-level role tests search and filtering against a large pool. The third layer is the agent's capability tags: multi-hop retrieval, multi-source verification, cross-domain retrieval, needle-in-a-haystack.

So a case is best written not as a simple prompt but as a coordinate: `task type × business scenario × capability tag`. The dataset is made of these coordinates, and when the question set grows, we can see which real scenarios are still missing; when a score moves, we know where it moved.

## 2. Every task type needs its own grading config

The dataset answers "what to evaluate." The grading config answers "how this type of task gets judged." We need to write down the expected result, the acceptable evidence, and the graders. Public agent benchmarks are converging the same way: deterministic checks when evidence suffices, model judgment only where semantics can't be enumerated. [Agents' Last Exam](https://arxiv.org/abs/2606.05405), [Claw-Eval-Live](https://arxiv.org/abs/2604.28139) and Anthropic's engineering guide on agent evals all use this division of labor.
...
| **L3 outcome** | Whether the delivery produced an effect in the real workflow. | Saves, accepts, contacts, replies, interviews — long-horizon observation of part of the online deliveries. Checks whether offline conclusions track user value. |
...
What matters most in a probe is being **hard to solve, easy to verify**: the agent must use the target capability to solve it, and the final result must be clearly checkable.
Multi-hop questions slip into multi-constraint questions easily: if every condition can be verified on the same profile, what it's really evaluating is L1 filtering, not a cross-entity, cross-source chain of information.

The other thing to watch is interference from the model's own knowledge. The LLM has already absorbed a huge amount of the world, so the target candidates we design shouldn't be prominent enough for the model to recite; keep at least one hop on recent evidence, isolated from training data, caches and the internal candidate store.
When pass rates approach saturation, there's no rush to swap metrics — add a hop, spread the sources wider, or raise distractor similarity. That keeps the probes pressure-testing capability, and keeps them working as the agent improves.

## 4. Absolute and relative: two measurement modes

The same grading config needs to run in two modes. Absolute evaluation answers "did it meet expectations"; relative evaluation answers "is it better than the baseline." Each has its uses — mix the two into one score and the results become hard to explain clearly.
...
**Relative evaluation**: which retrieval strategy brings back more usable candidates; which of two execution flows fits the JD better; is the new version better than the baseline. Right for choosing between approaches.

The trouble with open sourcing is that nobody knows every suitable candidate in the world, so "recall" as such can't be evaluated. The workable approach is to pool the candidates from several systems, add human judgment, and compute relative pool coverage.
...
1. **Calibrate the graders**: an L2 judge is a grader. The human gold set needs good cases as well as bad and boundary cases; compare the judge against experts on a regular basis.
...
Without human calibration, the L2 standard drifts; without environment and version records, historical scores can't be compared; without online flowback, offline evaluation never learns whether it stands for user value. Take away any one part and the others barely stand on their own.

## 6. An evaluation system is grown

When implementing, the first dataset doesn't need to be big. The most reliable source is production bad cases: a key person missed, a mismatch, a run that should have stopped and didn't, an empty delivery. These cases are themselves the most valuable increments the evaluation system gets.
The flow, roughly:
...
In the end, what an agent evaluation system produces is not just one overall score. What matters most is a durable ability to judge: we know which tasks got better, why we believe that conclusion, and what in the product to change next.
